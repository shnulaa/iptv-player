...
export const ChannelCard: React.FC<Props> = ({ channel, onPlay, onEdit, onDelete, onTest, testing }) => {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="channel-card glass rounded-xl overflow-hidden">
      <div className="p-4">
        <div className="flex items-start gap-3">
          <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-primary-500/20 to-purple-500/20 flex items-center justify-center flex-shrink-0">
            {channel.logo ? <img src={channel.logo} alt="" className="w-full h-full object-cover rounded-lg" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} /> : <Radio className="text-primary-400" size={24} />}
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="text-white font-medium truncate">{channel.name}</h3>
            <p className="text-gray-500 text-sm truncate">{channel.group_title}</p>
          </div>
          <div className="flex items-center gap-2">
            <div className={`status-indicator ${statusColors[channel.status]}`} />
            <span className="text-xs text-gray-400">{statusLabels[channel.status]}</span>
          </div>
        </div>
        <p className="mt-3 text-xs text-gray-500 truncate font-mono bg-dark-300 rounded-lg px-3 py-2">{channel.url}</p>
        {channel.response_time && <p className="mt-1 text-xs text-gray-500">响应: {channel.response_time}ms</p>}
      </div>
      <div className="flex border-t border-white/5">
        ...
        <button onClick={() => onTest(channel)} disabled={testing} className="flex-1 flex items-center justify-center gap-2 py-3 text-gray-400 hover:text-yellow-400 hover:bg-yellow-500/10 transition-colors disabled:opacity-50"><RefreshCw size={16} className={testing ? 'animate-spin' : ''} /><span className="text-sm">测试</span></button>
